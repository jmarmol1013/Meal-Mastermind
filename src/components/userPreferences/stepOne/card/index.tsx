...
export const Card: React.FC<Props> = ({ currentType, items, setUserType }) => {
    return items.map((item) => {
        return (
            <div
                className={`${currentType == item.type ? ' shadow-lg' : ''} flex w-1/2 flex-col rounded-md p-4 align-middle transition-all duration-200 ease-out hover:cursor-pointer hover:shadow-lg focus:shadow-lg`}
                key={item.type}
                onClick={() => setUserType(item.type)}
            >
                <h3 className="text-md text-fourth md:text-lg">{item.type}</h3>
